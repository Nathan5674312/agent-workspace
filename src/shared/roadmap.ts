...
export const ROADMAP: FeatureGroup[] = [
  {
    ...
    features: [
      ...
      {
        label: 'Clean modern UI (Notion-level polish)',
        status: 'built',
        note: "BUILT 2026-08-31, and the thing that had kept it partial is gone: no ribbon icon shows a panel naming an unbuilt feature any more. The previous note said five icons did; by the time it was checked, three did, because terminal and daily notes had already graduated. Each of the three was answered differently, and the differences are the point. SEARCH was BUILT: a real panel, see the Fast search entry. GRAPH was REPOINTED: its icon now opens the graph main view and keeps the sidebar you had, exactly as Versions already did, because the graph is the whole canvas and its filters, forces and display controls live in the view itself, so a sidebar copy would be a second set of the same knobs. PLUGINS was REMOVED: it was not an unbuilt feature waiting its turn but a promise the product had already decided not to keep, and an icon that opens a panel saying not built yet for something that will never be built is precisely the inert control this entry is about. Its roadmap entry stays; its place in the ribbon does not. The earlier audit still holds: every .tsx in src/renderer, 66 controls wired, 0 disabled, 4 read-only textareas that are read-only by design, and the nine in docs/buttons/ all built. Two tests hold this shut rather than an audit that decays: review-s2-vault-pane now walks every id in LeftRibbon and fails if the pane does not handle it by name, which is what makes a new icon impossible to add as a placeholder; and the older guard still fails any button with no onClick, disabled or popoverTarget. Watched running 2026-08-31 over CDP with real mouse input: every ribbon icon clicked in turn, zero placeholders.",
      },
      {
        label: 'Offline access',
        status: 'built',
        note: 'THE PREVIOUS NOTE WAS FALSE. It said "saving still calls a local HTTP server"; there is no local HTTP server and there has not been since `a826753`. Measured 2026-08-26: nothing under `src/` calls fetch, axios, XHR, WebSocket, `http.request` or `createServer` — the grep is empty — and the only two runtime dependencies are the Claude agent SDK and an icon set. Saving is `writeFileSync` to a temp file then `renameSync` over the target, so a crash mid-write cannot truncate a note. The renderer could not reach the network even if something tried: `index.html` ships `connect-src \'none\'` under `default-src \'none\'`, which the browser enforces rather than the code promising. THE ONE HONEST CAVEAT, and it is not the app: the agent host spawns the user\'s OWN installed CLI, which talks to whichever provider that user already pays for — their binary, their credential, their network call, on the same footing as the app opening a file they chose. As of 2026-09-08 nothing in the app starts one: `claude.ts` was the only caller and was deleted for being unreachable. Pull the plug and every local feature still works: read, edit, save, graph, database, canvas, versions, bookmarks, daily notes, block references.',
      },
      {
      ...
      {
        label: 'Open a note by name (Ctrl+P)',
        status: 'built',
        surface: 'Ctrl+P overlay',
        note: 'SHIPPED v1.1.0. A SECOND search surface rather than a mode of the Search panel, because it answers a different question: Fast search finds notes that SAY a thing, this one finds the note you can already NAME. Before it, reaching a note you could name meant scrolling the tree. Ctrl+P or Ctrl+O — both, because both are muscle memory — opens a box over the pane; type, arrow, Enter. It matches names against vault.tree(), which this process already holds, so it runs on every keystroke with NO disk read and no IPC round trip, and that is the whole reason it can be per-keystroke while the Search panel deliberately runs on Enter. The rule it lives or dies by is that an initialism works: nobody types roadmapStates to reach roadmapStates.ts, they type rmst and expect it first, so matching is subsequence and the SCORING is the real subject — a letter that starts a word counts for more than a letter inside one. Watched running 2026-09-10 on the packaged 1.1.0 build against the real 479-note vault: Ctrl+P opened the overlay over the editor, typing road returned 11 ranked results with the matched span highlighted in each name and the containing folder shown on the right, 02 - Build Roadmap first, and the footer read the keys back. 21 assertions in test/quickopen.test.mjs. NOT built: matching on path segments as well as the name, and any memory of what you opened last.',
      },
      {
        label: 'Fast search',
        status: 'partial',
        surface: 'sidebar:search',
        note: "The Search icon opens a real panel: a query box, results grouped by note, and clicking a line opens that note ON that line. Name matches rank first, then notes by how many lines matched, then path, so the order is stable between identical queries rather than depending on directory-walk timing. ~~Matching is case-insensitive substring~~ — CORRECTED 2026-09-10 in v1.1.0, and it was the bug rather than a detail: `fate food` searched for those nine characters in that order, so a note whose first line says Fate and whose fortieth says food did not match at all. A query is now the WORDS it is made of — a note matches when EVERY term appears in it somewhere, and a line is shown when it contains ANY of them, which is the rule people already know from the box they are switching away from. Quotes keep the old behaviour where it is wanted, so a quoted phrase is one term that must be found as written, and an unclosed quote is treated as closed at the end, because a person mid-type has an unclosed quote and refusing to search until they finish is worse than searching what they have. Bare terms under two characters are dropped; a quoted one is not. Measured on the 471-note vault: two words as a phrase returned 0 notes, the same two as words returned 7 in 196ms. The query is still NEVER compiled as a pattern, because people type `.` and `(` and a regex build would either match everything or throw. One hit per LINE, not per occurrence. PARTIAL FOR ONE HONEST REASON: it is a linear scan, not an index. vault.search re-reads the vault per query because scan() deliberately throws each note text away after extracting wikilink names, and its own comment says why (peak memory), so there was nothing cached to grep. Three things keep that bounded: a two-character floor shared with the main process so both agree what is worth running, a budget that stops the walk once enough notes have matched, and running on Enter rather than per keystroke. That last one is not a UX preference: this pane bans setTimeout outright, and weakening a real guard so new code can pass it is how a guard stops being one. Roadmap/02 still settles on SQLite FTS5 plus sqlite-vec, and search() in src/main/vault.ts is the one function that changes when it lands. It applies the SAME walk and exclusions as tree(), so a note the explorer hides, the database omits and the graph does not draw cannot surface here either. Watched running 2026-08-31 against the real vault. 46 assertions in test/search.test.mjs, plus 28 in test/search-words.test.mjs covering the word rule, the last of which runs six queries over 251 real notes and asserts all 2314 hit offsets slice back to the query, because an offset one out does not throw, it highlights the wrong word on every row.",
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      {
        label: 'Agents that read, write, and act on the vault',
        status: 'partial',
        surface: 'ribbon:terminal',
        note:
          "READ became READ AND WRITE on 2026-09-02, and the shape of it is the point. The BUILT-IN tool list is still ['Read', 'Glob', 'Grep'] — no Bash, and deliberately NOT the SDK's own Write and Edit, which write straight to disk and would walk past all four things vault.ts owns: the consent gate, the pre-edit backup under .backups/, the lost-update guard and the atomic temp-then-rename. Instead there are two in-process MCP tools, write_note and move_note, which do no filesystem work at all: each round-trips to the parent exactly as consent already did, and the parent calls vault.save/vault.move with an agent Actor. So agentHost.ts keeps its rule that the child never touches the vault, and an agent write is indistinguishable on disk from a user write except that it was gated. They are auto-approved at the SDK layer on purpose — consent.ts raises the real prompt, with the agent's own reason and the exact paths, and asking twice for one action is the approval fatigue that module opens by warning against. A refusal is reported to the model AS a refusal so it stops rather than retrying. Creating a folder stays a separate ask, because save() refuses to invent a directory from a typo'd path and that is tested. WHY PARTIAL AND NOT BUILT: 1049/1049 tests pass and test/agent-write.test.mjs covers the seven things that matter at the disk — create, refuse-and-write-nothing, backup-before-overwrite, the user's stale save raising SaveConflict instead of being clobbered, move, refused move, and a blank reason refused before it reaches a human — but NOBODY HAS WATCHED A REAL MODEL CALL THESE TOOLS IN THE RUNNING APP. Until someone has, the honest claim is that the write path works and the tools are wired, not that the feature is finished. Still missing after that: no delete tool by design, no Bash, and nothing acts on the Inbox's proposed filings on its own.",
      },
      {
      ...
      { label: 'Autonomous organization and curation', status: 'planned', note: 'Agents capture into `Inbox/` and propose a filing in the frontmatter of the capture itself. Nothing acts on those proposals, and as of 2026-09-06 nothing displays them either: the Inbox surface was removed rather than left as a view nobody opened. Whatever ships here needs its own surface, and the proposals are on disk waiting for it.' },
    ],
  },
  {
    title: 'Bookmarks',
  ...
  },
  {
    ...
    features: [
      ...
      { label: 'API / plugin ecosystem', status: 'planned', note: 'Kept on the list because it is what people ask for, but the entry above is the position: the agent is the plugin system and the canvas is the render target. Building a JS plugin host would be choosing Obsidian\'s supply-chain risk on purpose. NO LONGER HAS A RIBBON ICON, removed 2026-08-31: an icon whose panel says "not built yet" for something the position above says will never be built is an inert control, not a feature waiting its turn. The entry stays because the position is worth recording; what it no longer claims is a place in the ribbon. The extension story it points at is already on screen under Canvas, where boards an agent can run actually live.' },
