 ...
 * shared/canvas.ts.
 *
 * ── why this file is long, and stays long ──────────────────────────────
 *
 * It was 3204 lines. `CanvasList`, the sidebar's list of boards, has moved to
 * its own file: two components with two different consumers, sharing not one
 * constant, helper or line of state. That split was free.
 *
 * The remaining ~2750 are ONE component, and splitting it was considered and
 * rejected on the evidence rather than on taste:
 *
 *   - The pure logic is already gone. `shared/canvas.ts` is 695 lines with 21
 *     test files pointing at it, and `src/shared/` holds 6331 lines in total.
 *     What is left here is 42 hooks, 38 closures over that state, and 97 lines
 *     of JSX — pointer handling, undo, snapping and rendering, which is what a
 *     canvas editor's imperative layer IS.
 *   - Those closures read and write shared state. Prying them into separate
 *     components means prop-drilling or a context, and both ADD code. A split
 *     that makes the codebase bigger to make one file smaller is not a
 *     structural improvement, it is a line count moved sideways.
 *   - There is no safety net for it. Ten of 74 test files touch the renderer
 *     at all, and none exercise this component's body — the canvas suite tests
 *     `shared/canvas.ts`. Restructuring stateful pointer and undo code with no
 *     coverage is how a regression ships that only manual QA finds.
 *
 * The thing that would change this answer is coverage, not appetite: with the
 * drag, undo and snap paths under test, the sub-features (the edge layer, the
 * group layer, the selection model) have real seams. Until then the length is
